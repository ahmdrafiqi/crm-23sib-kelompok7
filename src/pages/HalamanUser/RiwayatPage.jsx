...
import { Package, Clock, CheckCircle, XCircle, Eye, Heart, Star, ShoppingBag } from 'lucide-react';
...
const RiwayatPage = () => {
  ...
  // Simulasi data pesanan yang lebih lengkap
  const sampleOrders = [
    {
      id: 'ORDER-1703001',
      userId: 1,
      items: [
        { productId: 1, productName: 'SKINTIFIC Aqua Light Sunscreen SPF 50', quantity: 1, price: 74100, image: '☀️' },
        { productId: 2, productName: 'Maybelline SuperStay Matte Ink Liquid Lipstick', quantity: 2, price: 89000, image: '💋' }
      ],
      totalAmount: 252100,
      status: 'delivered',
    ...
    },
    {
      ...
      userId: 1,
      items: [
        { productId: 3, productName: 'The Body Shop British Rose Body Lotion', quantity: 1, price: 159000, image: '🌹' },
        { productId: 4, productName: 'Cetaphil Gentle Skin Cleanser', quantity: 1, price: 125000, image: '🧴' }
      ],
      totalAmount: 284000,
      status: 'shipping',
    ...
    },
    {
      ...
      userId: 1,
      items: [
        { productId: 5, productName: 'COSRX Advanced Snail 96 Mucin Power Essence', quantity: 1, price: 185000, image: '🐌' },
        { productId: 6, productName: 'Urban Decay Naked Heat Eyeshadow Palette', quantity: 1, price: 650000, image: '🎨' }
      ],
      totalAmount: 835000,
      status: 'processing',
    ...
    },
    {
      ...
      userId: 1,
      items: [
        { productId: 7, productName: 'Wardah Perfect Bright Moisturizer', quantity: 2, price: 45000, image: '✨' },
        { productId: 8, productName: 'Pixy UV Whitening Two Way Cake', quantity: 1, price: 35000, image: '🧚' }
      ],
      totalAmount: 125000,
      status: 'cancelled',
    ...
    },
    {
      ...
      userId: 1,
      items: [
        { productId: 9, productName: 'Innisfree Green Tea Seed Serum', quantity: 1, price: 235000, image: '🍃' }
      ],
      totalAmount: 235000,
      status: 'delivered',
    ...
    },
    {
      ...
      userId: 1,
      items: [
        { productId: 10, productName: 'Laneige Water Sleeping Mask', quantity: 1, price: 320000, image: '💧' },
        { productId: 11, productName: 'Benefit Brow Precisely Eyebrow Pencil', quantity: 1, price: 450000, image: '✏️' }
      ],
      totalAmount: 770000,
      status: 'shipping',
  ...
  const statusOptions = [
    { value: 'all', label: 'Semua Status', count: sampleOrders.length, icon: ShoppingBag, color: 'text-gray-600' },
    { value: 'favorites', label: 'Suka', count: sampleFavorites.length, icon: Heart, color: 'text-pink-500' },
    { value: 'processing', label: 'Diproses', count: sampleOrders.filter(o => o.status === 'processing').length, icon: Clock, color: 'text-yellow-500' },
    { value: 'shipping', label: 'Dikirim', count: sampleOrders.filter(o => o.status === 'shipping').length, icon: Package, color: 'text-blue-500' },
    { value: 'delivered', label: 'Selesai', count: sampleOrders.filter(o => o.status === 'delivered').length, icon: CheckCircle, color: 'text-green-500' },
    { value: 'cancelled', label: 'Dibatalkan', count: sampleOrders.filter(o => o.status === 'cancelled').length, icon: XCircle, color: 'text-red-500' }
  ];
      ...
      {favorites.map((product) => (
        <div key={product.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
          ...
          <div className="relative">
            <div className="w-full h-48 bg-gradient-to-br from-pink-50 to-purple-50 flex items-center justify-center">
          ...
          <div className="p-4">
            ...
            <div className="flex items-center space-x-2 mb-4">
              <span className="text-lg font-bold text-pink-600">Rp{formatPrice(product.price)}</span>
              {product.originalPrice > product.price && (
                <span className="text-sm text-gray-500 line-through">Rp{formatPrice(product.originalPrice)}</span>
              )}
            </div>
    ...
    <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">
          {selectedStatus === 'favorites' ? 'Produk Favorit' : 'Riwayat Pesanan'}
        </h1>
          ...
          {statusOptions.map((status) => {
            ...
            return (
              <button
                ...
                className={`p-4 rounded-lg border-2 transition-all text-sm font-medium ${selectedStatus === status.value
                    ? 'border-pink-500 bg-pink-50 text-pink-700'
                    : 'border-gray-200 hover:border-pink-300 text-gray-700'
                  }`}
              >
                <div className="flex flex-col items-center space-y-2">
                  <IconComponent className={`w-5 h-5 ${selectedStatus === status.value ? 'text-pink-500' : status.color}`} />
      ...
      {selectedStatus === 'favorites' ? (
        <div>
          {favorites.length > 0 ? (
            ...
            <div className="text-center py-16">
              <div className="text-gray-400 text-6xl mb-4">💝</div>
              <h3 className="text-xl font-semibold text-gray-900 mb-2">Belum ada produk favorit</h3>
              <p className="text-gray-600 mb-6">Mulai eksplorasi dan tambahkan produk ke favorit Anda</p>
              <button
